...
void display()
{
	...
	glRotatef(-90, 1, 0, 0);
	glRotatef(g_RotationDegree, 0, 0, 1); // 回転させている
	...
	{
		const float thetaDegree = deltaTheta * i + g_RotationDegree; // ティーポットの位置を決めるための角度
		...
		// ティーポットの高さ方向の値
		const float yPos = g_HeightOffset + pow(sinf(thetaRad),2.0); // ★この値を少しずつ変化させることでティーポットが上下に移動する
...
void timer(int val)
{
	...
	g_EyeY = g_EyeCenterY + g_EyeRadius * sinf(rotationRad);
	g_EyeZ = g_EyeCenterZ + g_EyeRadius * sinf(rotationRad);
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("LA LA LA LOVE SONG feat. Tsukuba");
