...
double getBaseN(int i, int n, double t)
{
	...
	else
	{
		...
		{
			temp1 = ((t - g_NotVector[i]) / (g_NotVector[i + n] - g_NotVector[i]));
		}
		else
		{
			temp1 = 0;
		...
		{
			temp2 = ((g_NotVector[i + n + 1] - t) / (g_NotVector[i + n + 1] - g_NotVector[i + 1]));
		}
		else
		{
			temp2 = 0;
		}
		return temp1 * getBaseN(i, n - 1, t) + temp2 * getBaseN(i + 1, n - 1, t);
	}
}
...
void display(void)
{
	...
	glLineWidth(1);
	glBegin(GL_LINE_STRIP);
	for (unsigned int t = 4; t <= g_ControlPoints.size(); t += 1)
	{
		double px = 0, py = 0;
		for (unsigned int i = 0; i <= 3 + g_ControlPoints.size() - 1; i++)
		{
			px += getBaseN(i, 3, t) * g_ControlPoints[i].x;
			py += getBaseN(i, 3, t) * g_ControlPoints[i].y;
		}
		glVertex2d(px, py);
	}
	glEnd();
